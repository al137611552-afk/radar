...
.venv/bin/python option_cli.py \
  ...
  --alert-db output/alerts/alerts.db \
  --csv output/options_alerts.csv
```

首次使用会把当前全部命中标记为“首次命中”；之后相同信号保持静默。状态按扫描模式隔离，JSON采用原子替换写入，运行产物不会提交到Git。`--new-only` 还会先将增量事件幂等写入 `output/alerts/alerts.db`，再推进JSON状态；如果SQLite入箱失败，本轮直接失败且状态不会前进，调度重试时仍能重新产生事件。同一调度逻辑时点、合约和事件类型受唯一约束保护，不会因任务重试重复入箱；无调度逻辑时点的手工运行优先使用行情最新小时线时点作为幂等标识，缺少行情时点时才回退到上海当前整点，因此跨墙钟小时重试同一份行情也不会重复入箱。

自动调度会拆分保存四类期权产物：

- `output/options_candidates_latest.csv`：完整可分析候选池，可包含 `0 / 0` 的无信号合约；
- `output/options_latest.csv`：当前仍然有效的双确认信号，供Dashboard读取；
- `output/options_alerts.csv`：首次命中、新金叉、确认变化或失效等本轮增量告警；
- `output/alerts/alerts.db`：跨轮次持久化的可靠告警发件箱，供预警中心和Webhook投递器使用。
...
调度器会按上海时区运行三项行情任务，并在配置Webhook后额外运行可靠告警投递任务；各任务以“任务 + 逻辑时点”在SQLite中去重：

- `intraday`：日盘及夜盘每根完整5分钟线后运行；
- `options`：交易时段内每个完整时钟小时后运行；
- `momentum`：工作日15:00日盘收盘后运行；
- `alerts`：设置 `WATCHMAN_ALERT_WEBHOOK_URL` 后每分钟检查一次SQLite发件箱。
...
投递器对每条待处理告警使用SQLite事务claim和限时lease，成功后才标记 `delivered`。失败会释放lease并按 `60s、120s、240s…` 指数退避（单次最长24小时）；默认第5次失败后进入 `failed` dead-letter状态。进程在Webhook已接收、但本地成功标记前崩溃时可能进行至少一次重投，因此每个请求都携带由“来源 + 逻辑时点 + 实体 + 事件类型”计算的稳定 `Idempotency-Key`，接收端应按该键去重。多个投递进程可并发运行，过期lease会被安全回收，旧worker不能用过期token覆盖新worker的结果。

远程地址必须使用HTTPS；仅显式回环IP地址（例如 `127.0.0.1` 或 `[::1]`）允许HTTP，不信任可被本机解析配置改写的 `localhost` 主机名。Webhook请求显式禁用进程环境中的HTTP代理，避免回环明文请求和Bearer令牌被代理截获。URL禁止嵌入用户名或密码。认证令牌只从环境变量读取，不进入命令行、Webhook正文或Dashboard API：

```bash
export WATCHMAN_ALERT_WEBHOOK_URL='https://example.com/watchman-alerts'
export WATCHMAN_ALERT_WEBHOOK_TOKEN='你的Bearer令牌'  # 可选
.venv/bin/python alert_dispatch_cli.py
```

该命令单次最多处理一个有界批次，不会在进程内睡眠等待重试；后续到期重试由调度器下一分钟继续领取。`dead_lettered` 表示投递状态已被可靠持久化，是一次已处理的运行结果，因此CLI保持成功退出并通过JSON摘要及Dashboard暴露该计数；配置、数据库或进程级异常仍会非零退出。
...
2. 在 `/etc/watchman/watchman.env` 中配置 `QUOTE_API_KEY=...`；需要Webhook投递时再配置 `WATCHMAN_ALERT_WEBHOOK_URL=...` 和可选的 `WATCHMAN_ALERT_WEBHOOK_TOKEN=...`。目录和文件仅允许root读取；
...
项目提供零新增依赖的只读Web面板，聚合盘中成交额、期权信号、期权信号变化、品种动量、板块动量、日频排名变化、单品种研究详情、可靠预警中心、数据新鲜度与自动任务状态。面板每30秒自动刷新，支持手工刷新、功能页切换、代码/名称/板块/交易所搜索和桌面/移动端响应式布局。预警中心从SQLite发件箱展示最近100条事件，并使用全库聚合展示真实的待投递、已投递、失败数量，避免列表窗口截断积压统计；动量榜中的品种名称可直接点击进入详情页，也可从详情页选择器切换；详情聚合当前动量指标、最近60个交易日原始/风险调整多头排名轨迹、当前主力盘中状态和关联临期期权。
