...
项目提供零新增依赖的只读Web面板，聚合盘中成交额、期权信号、期权信号变化、品种动量、板块动量、日频排名变化、单品种研究详情、数据新鲜度与自动任务状态。面板每30秒自动刷新，支持手工刷新、功能页切换、代码/名称/板块/交易所搜索和桌面/移动端响应式布局。动量榜中的品种名称可直接点击进入详情页，也可从详情页选择器切换；详情聚合当前动量指标、最近60个交易日原始/风险调整多头排名轨迹、当前主力盘中状态和关联临期期权。
